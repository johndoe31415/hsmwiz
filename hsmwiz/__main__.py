...
def genparser(parser):
	...
	parser.add_argument("-v", "--verbose", action = "count", default = 0, help = "Increase verbosity. Can be specified multiple times.")
mc.register("verifypin", "Try to login a NitroKey by entering a PIN or SO-PIN", genparser, action = ActionVerifyPIN)
...
def genparser(parser):
	parser.add_argument("-s", "--subject", metavar = "subject", type = str, default = "/CN=NitroKey Example", help = "Specifies the CSR subject. Defaults to \"%(default)s\".")
...
def genparser(parser):
	parser.add_argument("-s", "--subject", metavar = "subject", type = str, default = "/CN=NitroKey Example", help = "Specifies the certificate subject. Defaults to \"%(default)s\".")
